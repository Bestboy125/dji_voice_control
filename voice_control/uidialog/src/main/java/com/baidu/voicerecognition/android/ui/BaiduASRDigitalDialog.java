...
//            setContentView(mContentRoot);
    ...
    protected void internalOnStart() {
        mTipsAdapter.clear();
        String[] temp = get1Params().getStringArray(PARAM_TIPS);
        if (temp != null) {
        ...
        if (mTipsAdapter.getCount() > 0) {
            ...
            showTips = get1Params().getBoolean(PARAM_SHOW_TIPS_ON_START, false);
        } else {
            mHelpBtn.setVisibility(View.INVISIBLE);
        }
        if (showTips) {
    ...
    protected void onPrepared() {
        ...
        mCompleteTextView.setEnabled(true);
        mHandler.removeCallbacks(mShowSuggestionTip);
        if (get1Params().getBoolean(PARAM_SHOW_TIP, true) && mTipsAdapter.getCount() > 0) {
        ...
        if (errorType != ERROR_NONE) {
            ...
            switch (errorType) {
                ...
                case SpeechRecognizer.ERROR_AUDIO:
                    mErrorRes = "启动录音失败";
                    if (mTipsAdapter.getCount() > 0) {
                        if (get1Params().getBoolean(PARAM_SHOW_HELP_ON_SILENT, true)) {
                            showHelpBtn = true;
                        }
                        if (get1Params().getBoolean(PARAM_SHOW_TIP, true)) {
